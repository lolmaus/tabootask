...
test('as an authenticated user, should be able to visit as logged in', async ({ page: p, baseURL, context }) => {
...
test('two users should be able to share the same DB', async ({ page: p, baseURL, browser, context }) => {
	...
	let user1: User, session1: Session, sessionToken1: string, user2: User, session2: Session, sessionToken2: string;
